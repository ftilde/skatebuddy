...
impl KalmanFilter {
    pub fn new() -> Self {
        Self {
            state_a_priori: Vector4::new(0.0, 0.0, 0.0, 0.0),
            p_a_priori: Matrix4::identity() * 100.0,
            t_in_s: 0.0,
        }
    }
    ...
    pub fn add_value(&mut self, measurement: RelativeNavigationData) -> State {
        ...
        //let dt = nt - self.t_in_s;
        let dt = 1.0; //TODO: something is broken here when we take the true time values. Probably
        ...
        //Correction
        let y_k = observation - self.state_a_priori;
        let S_k = self.p_a_priori + R_k;
        let K_k = self.p_a_priori * S_k.try_inverse().unwrap();

        let state = self.state_a_priori + K_k * y_k;
        let P = (Matrix4::identity() - K_k) * self.p_a_priori;

        //prediction
        self.state_a_priori = F * state;
        self.p_a_priori = (F * P * F.transpose()) + Q;
